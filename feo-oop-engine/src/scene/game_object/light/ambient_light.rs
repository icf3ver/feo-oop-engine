...
/// You should only use one of these but you can use as many as you want
#[derive(GameObject, Drawable, Parent, Child, Named, Scriptable)]
#[light] // TODO: I can do better ):
...
impl std::fmt::Debug for AmbientLight {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AmbientLight")
...
impl AmbientLight {
    ...
    pub fn new(
        ...
        engine_globals: EngineGlobals) -> Arc<RwLock<AmbientLight>> {
        let id = engine_globals.id_system.take();
        Arc::new(RwLock::new(AmbientLight {
            name: match name {
                Some(name) => name.to_string(),
                None => String::from("ambient_light_") + id.to_string().as_str()
            },
            id,
    ...
    fn draw<C, A, D> (
        ...
        diffuse_input: C,
        ambient_input: A,
        depth_input: D) -> AutoCommandBuffer
    where 
        C: ImageViewAbstract + Send + Sync + 'static,
        A: ImageViewAbstract + Send + Sync + 'static,
        D: ImageViewAbstract + Send + Sync + 'static,
    {
        let push_constants = fs_lighting_ambient::ty::PushConstants {
        ...
        let descriptor_set = PersistentDescriptorSet::start(layout.clone())
            ...
            .add_image(ambient_input).unwrap()
...
impl GraphicsSystemTrait for AmbientLight{
    ...
    where L: RenderPassAbstract + Sync + Send + 'static {
        let vertex_buffer = CpuAccessibleBuffer::from_iter(
                ...
                false,
                [
                    ScreenPos { position: [-1.0, -1.0] },
                    ScreenPos { position: [-1.0,  3.0] },
                    ScreenPos { position: [ 3.0, -1.0] },
                ].iter().cloned(),
        ...
        let pipeline = {
            ...
            let fs = fs_lighting_ambient::Shader::load(gfx_queue.device().clone())
                .expect("failed to create shader");
    ...
    fn get_system_num(&self) -> usize { 0 }
    ...
    fn pass<'b, 'p : 'b>(&self, pass_builder: &'b mut PassBuilder<'p>, gfx_system: GraphicsSystem){
        ...
        let command_buffer = self.draw(
            ...
            pass_builder.system.ambient_buffer.clone(),
            pass_builder.system.depth_buffer.clone(),
        );
